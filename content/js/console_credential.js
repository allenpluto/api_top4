...
var delete_credential_data = {
    ...
    'html_content':'<div class="overlay_content"><div class="overlay_info"></div><div class="delete_confirm_message_container"><p>Once you delete the credential, all API calls associated with this API-KEY would not be able to send request.</p><p>The process is irreversible, ARE YOU SURE?</p></div><div class="delete_confirm_button_container"><div class="delete_confirm_button_submit general_style_input_button general_style_input_button_gray">Yes</div><div class="delete_confirm_button_reset general_style_input_button general_style_input_button_gray overlay_close">Cancel</div></div></div>',
            ...
            $.ajax({
                ...
                if (status == 'success')
                {
                    ...
                    else
                    {
                        ...
                        {
                            overlay_info.removeClass('overlay_info_success').addClass('overlay_info_error').html(data.message+' <a href="javascript:location.reload();">Refresh Page</a> to continue');
                        }
                        else
                        {
                            overlay_info.removeClass('overlay_info_success').addClass('overlay_info_error').html(data.message);
                        }
                    }
                }
                else
...
var update_credential_data = {
    ...
    'init_function':function(overlay_trigger){
        ...
        if (api_key_container.data('new_key') === true)
        {
            api_key_container.data('new_key',false);

            var api_index = api_key_container.parent().find('.api_key_container').length - 1;
            update_form.find('input[name="alternate_name"]').val('API Key '+api_index);
            var ip_row = $('<div />',{
                'class':'ip_row_container'
            }).html('<div class="ip_value">'+$('input[name="remote_ip"]').val()+'</div><div class="ip_delete">&#xf00d;</div>').appendTo(update_form.find('.update_row_ip_container'));
        ...
        $('.ip_add_new').keydown(function(event){
            switch (event.which)
            {
                case 13:
                    ...
                    {
                        overlay_info.removeClass('overlay_info_success').addClass('overlay_info_error').html('IP Address is invalid');
                        return false;
                    }
        ...
        $('.update_form_button_submit').click(function(){
            ...
            post_value['ip_restriction'] = ip_restriction.join();
...
console.log($('base').attr('href'));
            if (data.status == 'OK')
            {
                ...
                for(var i=0;i<result_length;i++)
                {
                    ...
                    api_key_container.data('new_key',true);
